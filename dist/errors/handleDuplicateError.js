...
const handleDuplicateError = (error) => {
    ...
    // Error message looks like: 'E11000 duplicate key error collection: test.users index: email_1 dup key: { email: "test@test.com" }'
    const match = error.message.match(/"([^"]*)"/);
    const extractedValue = match && match[1]; // Get the duplicate value
    // Get the field name from error
    const fieldName = Object.keys(error.keyValue)[0]; // Usually "email", "phone", etc.
    const errorSources = [
        {
            path: fieldName,
            message: `${extractedValue} already exists`, // "test@test.com already exists"
        },
    ];
    return {
        statusCode: 400,
